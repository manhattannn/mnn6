	...
	function loadSchedule(date){
		$.ajax({
			...
			success: function(data){
				...
				// weekday show rows
				for (var i = 1; i <= 5; i++){
					var wdShowRow = '<div class="row row-shows row-weekday">';
					...
					for (var j = 0; j < numShows; j++) {
						...
						wdShowRow += '<div class="'+ cls +'"><div class="inner">'+link+category+'</div></div>';
					}
					wdShowRow += '</div> '
					wdRows += wdShowRow
				}
				...
				// weekend show rows
				for (var i = 6; i <= 7; i++){
					var weShowRow = '<div class="row row-shows row-weekend">';
					...
					for (var j = 0; j < numShows; j++) {
						...
						day[j].is_today == 'true' ? current = ' current' : current = '';
						var cls = 'cell cell-' + j + ' t' + day[j].duration + ' s' + day[j].start + current;
						weShowRow += '<div class="'+ cls +'"><div class="inner">'+link+category+'</div></div>';
					}
					weShowRow += '</div> '
					weRows += weShowRow
				}
