...
// Add Zen Tabs styles
if (theme_get_setting('html5_base_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'html5_base') .'/css/tabs.css', 'theme', 'screen');
}
...
function html5_base_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = html5_base_id_safe('node-type-' . $vars['type']);
  $vars['classes'] = implode(' ', $classes); // Concatenate with spaces
}

function html5_base_preprocess_comment_wrapper(&$vars) {
  $classes = array();
...
function html5_base_preprocess_block(&$vars, $hook) {
  $block = $vars['block'];

  // special block classes
  $classes = array('block');
  $classes[] = html5_base_id_safe('block-' . $vars['block']->module);
  $classes[] = html5_base_id_safe('block-' . $vars['block']->region);
  if(isset($vars['block']->bid)) {
    $classes[] = html5_base_id_safe('block-id-' . $vars['block']->bid);
  }
  $classes[] = 'clearfix';
  ...
  if (theme_get_setting('html5_base_block_editing') && user_access('administer blocks')) {
...
function html5_base_preprocess_comment(&$vars, $hook) {
...
function html5_base_menu_item_link($link) {
...
function html5_base_menu_local_tasks() {
  $output = '';
...
function html5_base_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
  ...
  $css_class = html5_base_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function html5_base_id_safe($string) {
...
*/
function html5_base_breadcrumb($breadcrumb) {
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('html5_base_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('html5_base_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('html5_base_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('html5_base_breadcrumb_title')) {
        if ($title = drupal_get_title()) {
          $trailing_separator = $breadcrumb_separator;
        }
      }
      elseif (theme_get_setting('html5_base_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
